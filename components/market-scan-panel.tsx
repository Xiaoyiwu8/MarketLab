...
export default function MarketScanPanel({onSelect}:{onSelect:(series:Series)=>void}){
  ...
  return <section className="panel" style={{border:'2px solid #4f9b86',marginBottom:24}}>
    <p className="eyebrow">DAILY US MARKET SCAN · PAPER ONLY</p><h2>每日全市场选股 · 做多2只 / 做空2只</h2>
    ...
    {scan&&<><h3>交易日 {scan.date} · {scan.status==='complete'?'已完成全名单尝试':'扫描尚未完成'}</h3>
      ...
      {scan.status==='complete'?<CandidatePanel data={results} busy={false} onSelect={symbol=>{const s=results.find(x=>x.symbol===symbol);if(s)onSelect(s);}} />:<p>完成全名单检查后才展示最终两多两空，避免把先扫描到的股票误认为全市场最佳候选。</p>}
      {scan.failed>0&&<details><summary>查看失败原因（前100条）</summary><ul>{scan.errors.map(e=><li key={e.symbol}>{e.symbol}：{e.reason}</li>)}</ul></details>}
      <details><summary>名单来源与筛选口径</summary><p>{scan.directoryStamps.join(' / ')}</p><a href="https://www.nasdaqtrader.com/trader.aspx?id=symboldirdefs" target="_blank" rel="noreferrer">Nasdaq 官方目录说明</a><p>趋势、成交量、RSI、支撑压力和收益风险条件全部满足后，按参考收益风险比排序。排序不是胜率；入场仍需核对当前价格及事件。做空标的不代表券商有可借股票。</p></details>
    </>}
