...
import ScanExamples from './scan-examples';


import type {EarningsWeek} from '@/lib/earnings-calendar';
import type {LogicState} from '@/lib/logic';
export default function MarketScanPanel({onSelect,earnings,logic}:{onSelect:(series:Series)=>void;earnings?:EarningsWeek|null;logic?:LogicState}){
  ...
  const results=scan?(scan.representatives??[...scan.long,...scan.short]):[];
  return <section className="panel" style={{border:'2px solid #4f9b86',marginBottom:24}}>
    <p className="eyebrow">DAILY US MARKET SCAN · PAPER ONLY</p><h2>每日全市场代表股票 · 左侧2多2空 / 右侧2多2空</h2>
    ...
    {scan&&<><h3>交易日 {scan.date} · {scan.status==='complete'?'已完成全名单尝试':'扫描尚未完成'}</h3>
      ...
      {scan.ruleVersion===SIGNAL_VERSION&&<ScanExamples data={results} onSelect={onSelect} complete={scan.status==='complete'}/>}
      {scan.rejections&&<details><summary>展开数量统计与排除原因</summary><p>每只有效股票在每组记录首个未通过条件；符合条件数为排序前数量，不等于最终名额。尚在扫描时统计为当前进度。</p>{Object.entries(scan.rejections).map(([group,reasons])=><div key={group}><h4>{group}</h4><ul>{Object.entries(reasons).sort((a,b)=>b[1]-a[1]).map(([reason,count])=><li key={reason}>{reason}：{count} 只</li>)}</ul></div>)}</details>}
      {scan.ruleVersion!==SIGNAL_VERSION?<p>旧版结果已停用。请开始扫描，按左侧 / 右侧四组规则重新检查全名单。</p>:scan.status==='complete'?<CandidatePanel logic={logic} earnings={earnings} data={results} busy={false} onSelect={symbol=>{const s=results.find(x=>x.symbol===symbol);if(s)onSelect(s);}} />:<p>完成全名单检查后才展示四组候选（每组最多2只），避免把先扫描到的股票误认为全市场代表结果。</p>}
