...
export async function POST(req:Request){
  try{
    ...
    if(action==='start'){
      ...
      if(existing)return Response.json({scan:JSON.parse(existing.state)},{headers});
      const universe=await stockUniverse();
      const scan:ScanState={date:target,startedAt:new Date().toISOString(),updatedAt:new Date().toISOString(),status:'running',total:universe.listings.length,cursor:0,analyzed:0,ineligible:0,failed:0,excluded:universe.excluded,directoryStamps:universe.stamps,errors:[],long:[],short:[]};
    ...
    try{
      ...
      const next=state.status==='complete'?state:await advanceScan(state,JSON.parse(row.listings) as Listing[]);
