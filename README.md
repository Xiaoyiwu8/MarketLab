## Dynamic logic and evidence (2026-09-12)

Scan progress now leads with one concrete example per strategy/direction from the retained representative pool; count diagnostics are collapsed. Examples identify passed conditions, the first rejection and missing confirmations, and link to the actual series. They are provisional during scanning, may change, and are not new entry recommendations or exemplars of every excluded ticker.

Automatic correction is deliberately bounded: recognized official HTML sources are fingerprinted at manual review, then rechecked while the page is visible. Body changes, access failure or a stale check automatically suspend reliance on that interpretation in both directions. Changed baselines require a new review; old versions remain. Checks are stored separately in `logic_source_checks` (`drizzle/0002_logic_source_checks.sql`). Each call checks up to six due sources; excess records remain gated when their checks go stale. A fingerprint change may be a page edit, not an economic revision; the system does not invent a replacement causal story. Unsupported/blocked sources remain explicitly manual. Review-only evidence expiry also triggers automatic suspension. Source checks do not replace the underlying source's publication date. Price structure is recalculated on the loaded daily bars, not live intraday ticks.

Three explicit hypotheses (macro, AI cash conversion, and shipping/energy) now have an evidence review panel. Fixed Fed monetary-policy RSS, EIA Today in Energy RSS, and a separately labeled Google News AI search supply titles for review. These are discovery feeds, not automatically verified causal evidence. The visible page checks every five minutes and on return to the tab; browser throttling and provider delays apply. No background LLM, guaranteed real-time coverage, external notifications or unattended server scheduler is enabled.

Manual reviews separate sourced facts from interpretations, ticker scope, publication time, review timestamp, expiry, confirming/disconfirming stance and direction-specific entry restrictions. D1 stores append-only revisions per authenticated owner; localhost uses a separate development identity and database. A unique owner/thesis/revision key prevents simultaneous edits from silently replacing evidence. History is paginated. Apply `drizzle/0001_logic_evidence.sql` to local D1 before using the new panel; production applies this migration during publication. Never modify an already-applied migration.

Candidate cards and the active equity's logic panel share these restrictions. Expired blocking evidence stays blocked pending a new review; unavailable evidence storage suppresses entry plans rather than assuming safety. Explicitly clearing one thesis does not clear other restrictions. Positive evidence does not create an entry, increase technical percentages or bypass existing chart/event checks. This is an advisory display gate, not an order-execution or existing-position exit rule. The existing simulator, general technical panels and stock-scanner ranking remain separate. Crypto is not assigned the equity evidence gate. There are no invented probabilities, automatic causal conclusions or seeded live evidence.
...
保留左多、左空、右多、右空各最多2只。右侧逐个检查独立突破，旧失败结构不再阻止检查后续结构；确认日起最多3个交易日有效，始终检查原边界、原止损、原第一目标、当前入场距离与收益风险。止损与目标按确认当时的信息固定，确认日量能用于验证，不能用后来的低量否定原放量确认。

左侧检查最近5个交易日的试探形态，新增低位支撑收阳企稳与高位压力收阴回落，不再强制假突破；仍要求已有40日横盘、高低位置、独立后续日线确认、原结构未被破坏、目标未触及，以及收益风险比至少2。确认后的有效窗口同样最多3个交易日。日线工程规则尚未经过盈利回测，更多形态不保证更多收益，也不保证每天凑满8只。

全市场扫描按每只股票在每组的首个未通过条件累计排除原因，符合条件数为排名前数量。数据与流动性均未通过时明确提示没有有效样本，不把数据失败解释为市场没有机会。升级后旧记录自动要求重新扫描，下面旧版规则描述仅作为历史记录。

## 左侧 / 右侧四组推荐（four-groups-v3）

每日左侧最多2只做多、2只做空，右侧最多2只做多、2只做空，合计最多8只，各组独立排序且不跨组补位，每只显示左侧/右侧及具体策略；同一方向同一股票仅占一个名额。右侧沿用下方突破回踩规则。左侧新增40日横盘后的低位假跌破收回（黄金坑形态候选），以及高位假突破收回后的压力反转做空，均要求后续独立日线确认而非仅凭低价或高价入选。横盘区间必须在此前120日范围的低35%或高35%；试探幅度不超过0.75 ATR，试探日或确认日量比至少1.2，以原区间中位价为第一目标，收益风险比至少2。

量价形态不能证明主力吸筹、散户割肉或主力出货；不把这些参与者身份作为事实展示。左侧尚未完成趋势确认，可能继续下跌/上涨。排序只比较参考收益风险比，非胜率或回测结果；新规则参数尚未经过盈利验证。升级后须重新扫描全名单，旧版结果不可直接作为新版完整候选。若两类同时满足，优先展示右侧标签。

## 突破回踩确认规则（retest-v1）

每日推荐要求先突破固定的前20日区间，之后独立日线回踩守住边界，再由后续日线确认。返回原区间使本次结构失效；突破尝试最多观察15根日线，确认后不重复追价，距边界超过1个突破前区间ATR时不入选。区间方向效率须不超过0.35，宽度不超过中价20%。保留原趋势、量能和RSI过滤；仅用突破之前已确认的历史拐点作目标，缺少目标或收益风险比不足2时不推荐。多空对称，不凑满名额。

这是日线工程近似，非严格缠论三买/三卖，参数尚未经盈利回测验证。旧扫描结果停用，并在继续扫描时重新检查全名单。单股详情中的基础买卖观察不等于本推荐模块的成熟确认。本规则不修改已有仓位的止损。
