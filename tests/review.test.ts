...
test('short signals use stop above entry, block oversold chasing and cover upward breaks',()=>{
  const bars=Array.from({length:61},(_,i)=>bar(new Date(Date.UTC(2026,6,6+i)).toISOString().slice(0,10),100));
  bars[60]={...bars[60],open:95,high:96,low:89,close:90,volume:300};
  const series={symbol:'TEST',bars,source:'test fixture',asOf:bars[60].date,adjustment:'none',warnings:[]};
  const now=new Date('2026-09-06');
  const s=shortGuidance(series,now)!;assert.ok(s.stop>90);assert.ok(s.target<90&&s.target>0);assert.equal(s.enter,false);assert.equal(s.checks[3].pass,false);
  bars[60]={...bars[60],open:105,high:111,low:104,close:110};assert.equal(shortGuidance(series,now)!.cover,true);
  assert.equal(shortGuidance({...series,source:'合成演示'},now)!.cover,false);
});
